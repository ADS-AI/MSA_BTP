...
from ...utils.text.train_utils import query_data_in_victim, active_train , evaluate, get_entropy, set_seed, agreement_score, get_label_probs, get_index_by_vote, get_softmax
...
def get_max_distance_point(cluster_centres_softmax_dict, unlabeled_softmax_dict):
    max_distance = 0
    max_distance_point = None

    for unlabelled_index , unlabeled_point in unlabeled_softmax_dict.items():
        # Calculate the distance to each cluster center
        distances = []
        for cluster_index , cluster_center in cluster_centres_softmax_dict.items():
            distances.append(euclidean_distance(unlabeled_point, cluster_center))

        # Find the maximum distance and corresponding point
        current_max_distance = max(distances)   

        if current_max_distance > max_distance:
            max_distance = current_max_distance
            max_distance_point = unlabelled_index

    return unlabelled_index

def k_centre_stealing(cfg, victim_dataset, thief_dataset, victim_model, thief_model, victim_tokenizer, thief_tokenizer, victim_config, thief_config):
    ...
    cluster_centres = labelled_index_list + validation_index_list
    ...
    cluster_points_dataset = thief_dataset.get_loaded_features(index = cluster_centres , split = "train" , true_labels = true_train_labels_thief)
    ...
    cluster_points_loader = DataLoader(cluster_points_dataset, sampler=SequentialSampler(cluster_points_dataset), batch_size=BATCH_SIZE)
    ...
    unlabelled_loader = DataLoader(unlabelled_dataset, sampler=SequentialSampler(unlabelled_loader), batch_size=BATCH_SIZE)
    cluster_centres_softmax_list = get_softmax(cfg , thief_model , thief_tokenizer , thief_config , cluster_points_loader)
    unlabeled_softmax_list = get_softmax(cfg , thief_model , thief_tokenizer , thief_config , unlabelled_loader)
    max_distance_index = get_max_distance_point(cluster_centres_softmax_list, unlabeled_softmax_list)
    ...
    for cycle in range(1 , total_cycles + 1):
        ...
        if cycle == total_cycles:
            for i in range(int(budget_last_cycle)):
                max_distance_index = get_max_distance_point(cluster_centres_softmax_list, unlabeled_softmax_list)
                cluster_centres_softmax_list[max_distance_index] = unlabeled_softmax_list[max_distance_index]
                unlabeled_softmax_list.pop(max_distance_index)
                cluster_centres.append(max_distance_index)
                index_to_query.append(max_distance_index)
            budget_used += budget_last_cycle
            budget = budget - budget_last_cycle
        else:
            for i in range(int(budget_per_cycle)):
                max_distance_index = get_max_distance_point(cluster_centres_softmax_list, unlabeled_softmax_list)
                cluster_centres_softmax_list[max_distance_index] = unlabeled_softmax_list[max_distance_index]
                unlabeled_softmax_list.pop(max_distance_index)
                cluster_centres.append(max_distance_index)
                index_to_query.append(max_distance_index)
            budget_used += budget_per_cycle
            budget = budget - budget_per_cycle

        unlabeled_index_list = list(set(unlabeled_index_list) - set(index_to_query))
        labelled_index_list.extend(index_to_query)
...
def k_centre_qbc_stealing(cfg, victim_dataset, thief_dataset, victim_model, victim_tokenizer, victim_config, list_models : list, list_tokenizers : list , list_configs : list):
