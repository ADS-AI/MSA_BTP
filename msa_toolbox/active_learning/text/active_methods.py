...
def entropy_stealing(cfg, victim_dataset, thief_dataset, victim_model, thief_model, victim_tokenizer, thief_tokenizer, victim_config, thief_config):
    ...
    budget_used = 2000
    budget = budget - 2000
    budget_per_cycle = budget / (total_cycles - 1)
    budget_last_cycle = budget - (total_cycles - 2) * budget_per_cycle
    budget_per_cycle = int(budget_per_cycle)
    budget_last_cycle = int(budget_last_cycle)
    print("budget_per_cycle", budget_per_cycle)
    print("budget_last_cycle", budget_last_cycle)

    for cycle in range(1 , total_cycles + 1):
        model_cycle = copy.deepcopy(thief_model)
        model_cycle.to(device)
        tokenizer_cycle = copy.deepcopy(thief_tokenizer)
        config_cycle = copy.deepcopy(thief_config)
        print("--------------- cycle {} budget used - {}---------------".format(cycle , budget_used))
        training_dataset = thief_dataset.get_loaded_features(index = labelled_index_list , split = "train" , true_labels = true_train_labels_thief, features = thief_dataset_train_features_for_thief)
        validation_dataset = thief_dataset.get_loaded_features(index = validation_index_list , split = "train" , true_labels = true_train_labels_thief, features = thief_dataset_train_features_for_thief)
        ...
        global_step, tr_loss , validation_accuracy , model_cycle , config_cycle , tokenizer_cycle = active_train(cfg , train_loader , validation_loader , model_cycle , config_cycle , tokenizer_cycle)
        ...
        save_thief_model(model_cycle , tokenizer_cycle , config_cycle, model_out_dir + "/{}".format(cfg.THIEF.ARCHITECTURE))
        save_thief_model(model_cycle , tokenizer_cycle , config_cycle, model_out_dir + "/cycle-{}".format(cycle))
        
        # prepare for middle cycles
        if cycle == total_cycles:
            unlabelled_dataset = thief_dataset.get_loaded_features(index = unlabelled_index_list , split = "train" , features = thief_dataset_train_features_for_thief)
            ...
            index_to_entropy = get_entropy(cfg , model_cycle , tokenizer_cycle , config_cycle , unlabelled_loader)
            ...
            print("--------------- querying victim model ---------------")
            thief_dataset_train_for_victim = thief_dataset.get_loaded_features(index = indexes_to_query , split = "train" , features = thief_train_features_for_victim_model)
            thief_train_dataloader_for_victim = DataLoader(thief_dataset_train_for_victim, sampler=SequentialSampler(thief_dataset_train_for_victim), batch_size=BATCH_SIZE)
            true_train_labels_thief_sub , entropy_train_thief_sub , index_list_train_sub = query_data_in_victim(cfg=cfg, theif_dataloader=thief_train_dataloader_for_victim, victim_model=victim_model, victim_tokenizer=victim_tokenizer, victim_config=victim_config)
            true_train_labels_thief.update(true_train_labels_thief_sub)
            entropy_train_thief.update(entropy_train_thief_sub)
    ...
    for i , model_folder in enumerate(model_folders_paths):
        ...
        print("metrics - ", metrics_list[model_folders[i]])
...
def qbc_stealing(cfg, victim_dataset, thief_dataset, victim_model, victim_tokenizer, victim_config, list_models : list, list_tokenizers : list , list_configs : list):
    ...
    for cycle in range(1 , total_cycles + 1):
        for key in list_models.keys():
            cycle_model = copy.deepcopy(list_models[key])
            cycle_model.to(device)
            cycle_tokenizer = copy.deepcopy(list_tokenizers[key])
            cycle_config = copy.deepcopy(list_configs[key])
            ...
            train_loader = DataLoader(training_dataset, sampler=RandomSampler(training_dataset, generator=torch.Generator().manual_seed(cfg.SEED)), batch_size=BATCH_SIZE)
            validation_loader = DataLoader(validation_dataset, sampler=SequentialSampler(validation_dataset), batch_size=BATCH_SIZE)
            global_step, tr_loss , validation_accuracy , cycle_model , cycle_config , cycle_tokenizer = active_train(cfg , train_loader , validation_loader , cycle_model , cycle_config , cycle_tokenizer)
            ...
            save_thief_model(cycle_model , cycle_tokenizer , cycle_config, sub_model_out_dir + "/cycle-{}".format(cycle))
            save_thief_model(cycle_model , cycle_tokenizer , cycle_config, sub_model_out_dir + "/{}".format(cfg.THIEF.ARCHITECTURE))

            if cycle <= total_cycles:
                unlabelled_dataset = thief_dataset.get_loaded_features(index = unlabelled_index_list , split = "train" ,  features = thief_train_features_per_model[key])
                unlabelled_loader = DataLoader(unlabelled_dataset, sampler=SequentialSampler(unlabelled_dataset), batch_size=BATCH_SIZE)
                index_to_label_prob[key] =  get_label_probs(cfg , cycle_model , cycle_tokenizer , cycle_config , unlabelled_loader)
            else :
                pass
        ...
        if cycle == total_cycles:
            ...
            indexes_to_query = unlabelled_index_list[:budget_last_cycle]
            unlabelled_index_list = unlabelled_index_list[budget_last_cycle:]
            budget_used = budget_used + budget_last_cycle
            budget = budget - budget_last_cycle   
            labelled_index_list.extend(indexes_to_query)
            print("--------------- querying victim model ---------------")
    ...
    print(validation_accuracy_dict_all_models)
    # with open(os.path.join(model_out_dir, "validation_results.txt"), "w") as f:
    #     f.write("cycle\tglobal_step\ttraining_loss\tvalidation_accuracy\n")
    ...
    for key in list_models.keys():
        ...
        for i , model_folder in enumerate(list_model_paths):
            ...
            print(key , " metrics - ", metrics_list[key][i])
