...
def start_training(config_name):
    ...
    if config_path in os.listdir(config_dir):
        app(config_path)
    else:
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("Generate Config"):
        input = [
        gr.Textbox(lines=1, label="Config Name", info="Enter the Config Name"),
        ...
        gr.Dropdown(
            ['cross_entropy_loss','mse_loss','l1_loss','soft_margin_loss','bce_loss'],
            label="Creteria", 
    ...
    with gr.Tab("Start Tranning"):
        with gr.Column():
            output = gr.Textbox(lines=15, label="Configs", info="Available Configs")
            config_button = gr.Button("Update Configs")
            
        with gr.Column():
            inputs = gr.Textbox(lines=1, label="Config", info="Name of the Config file")
            output_tran = gr.Textbox(lines=15, label="Progress", info="Progress of the Training")
...
demo1 = gr.Interface(count, inputs=None, outputs =gr.Textbox(label="Output Box"))
